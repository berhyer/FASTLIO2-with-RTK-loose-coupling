...
public:
    ...
    struct KD_TREE_NODE
    {
        PointType point; // 当前节点存储的物理点云数据
        int division_axis; // 当前节点的空间划分轴：0代表X轴, 1代表Y轴, 2代表Z轴
        int TreeSize = 1; // 以当前节点为根的子树下包含的总节点数 (含已删除节点)
        int invalid_point_num = 0; // 子树下被打上惰性删除标记的无效点数量
        int down_del_num = 0; // 降采样过程中被删除的点数量

...
void LidarProcessor::process(SyncPackage &package)
{
    ...
    int n_points = m_cloud_down_lidar->size();
    if (n_points > m_point_selected_flag.size()) {
        m_cloud_down_world->resize(n_points);
        m_norm_vec->resize(n_points);
        m_effect_cloud_lidar->resize(n_points);
        m_effect_norm_vec->resize(n_points);
        m_nearest_points.resize(n_points);
        m_point_selected_flag.resize(n_points);
    }
    trimCloudMap();
    m_kf->update();
    incrCloudMap();
...
#pragma omp parallel for
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        m_point_selected_flag[i] = false;
        if (esti_plane(points_near, 0.18, pabcd))
        {

...
struct NodeConfig
{
    ...
    // --- 新增：雷达类型参数 ("livox", "velodyne", "ouster", "generic") ---
    std::string lidar_type = "livox";
    // --- 新增: IMU 加速度单位缩放因子 ---
...
public:
    LIONode() : Node("lio_node")
    {
        ...
        m_imu_sub = this->create_subscription<sensor_msgs::msg::Imu>(m_node_config.imu_topic, imu_qos, std::bind(&LIONode::imuCB, this, std::placeholders::_1));
        ...
        {
            // 如果是原生大疆雷达，订阅 CustomMsg，绑定专用回调 livoxCB
            m_livox_sub = this->create_subscription<livox_ros_driver2::msg::CustomMsg>(
                m_node_config.lidar_topic, lidar_qos, std::bind(&LIONode::livoxCB, this, std::placeholders::_1));
            RCLCPP_INFO(this->get_logger(), "Subscribed to Livox CustomMsg on topic: %s", m_node_config.lidar_topic.c_str());
        } 
        else 
        {
            // 如果是 Velodyne/Ouster/通用点云，订阅 PointCloud2，绑定通用回调 lidarCB
            m_lidar_sub = this->create_subscription<sensor_msgs::msg::PointCloud2>(
                m_node_config.lidar_topic, lidar_qos, std::bind(&LIONode::lidarCB, this, std::placeholders::_1));
            RCLCPP_INFO(this->get_logger(), "Subscribed to standard PointCloud2 on topic: %s", m_node_config.lidar_topic.c_str());
        }
    ...
    {
        CloudType::Ptr cloud = Utils::commonLidar2PCL(msg, m_builder_config.lidar_filter_num, 
    ...
    void livoxCB(const livox_ros_driver2::msg::CustomMsg::SharedPtr msg)
    {
...
private:
    ...
    rclcpp::Subscription<livox_ros_driver2::msg::CustomMsg>::SharedPtr m_livox_sub;
